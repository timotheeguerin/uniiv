class User::Advisor < User
  ...
  def student_with_most_issues
    User::Student.joins(:advisor_students).joins(:issues)
    .where(issue_issues: {assignee_id: id, status: Issue::Issue.statuses[:open]},
           user_advisor_students: {status: User::AdvisorStudent.statuses[:validated]})
    .group('id')
    .having('count(*) > 0')
    .order('count(*) desc')
  end
